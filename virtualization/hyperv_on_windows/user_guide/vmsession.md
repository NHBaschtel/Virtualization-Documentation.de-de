...
> Hinweis: Wenn keine Verbindung für die Sitzung hergestellt werden kann, lesen Sie den Abschnitt [Problembehandlung](vmsession.md#troubleshooting), um mögliche Ursachen herauszufinden. 
...
2. Führen Sie einen der folgenden Befehle aus, um mit `New-PSSession` eine permanente PowerShell-Sitzung auf dem virtuellen Computer zu erstellen.
  ...
  > **Warnung:**  
   Builds vor Buildnummer 14500 weisen einen Fehler auf.  Wenn Anmeldeinformationen nicht explizit mit dem Flag `-Credential` angegeben werden, stürzt der Dienst im Gastsystem ab und muss neu gestartet werden.  Wenn dieses Problem auftritt, finden Sie [hier](vmsession.md#error-a-remote-session-might-have-ended) Anweisungen zur Umgehung.
