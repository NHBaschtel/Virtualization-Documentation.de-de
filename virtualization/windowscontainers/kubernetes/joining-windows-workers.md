...
ms.locfileid: "9001616"
---
# <a name="joining-windows-server-nodes-to-a-cluster"></a>Hinzufügen von Windows Server-Knoten zu einem Cluster #
Wenn Sie das [Einrichten eines Kubernetes-master-Knotens](./creating-a-linux-master.md) und [Ihre Lösung für die gewünschte Netzwerk ausgewählt](./network-topologies.md)haben, können Sie Windows Server-Knoten zu einem Cluster beitreten. Dies erfordert einige [Vorbereitung auf den Windows-Knoten](#preparing-a-windows-node) , bevor Sie einbinden.
...
Um Kubernetes ausführen zu können, müssen Sie zuerst herunterladen der `kubectl`, `kubelet`, und `kube-proxy` Binärdateien. Sie können diese über die Links im Herunterladen der `CHANGELOG.md` Datei mit den [neuesten Versionen](https://github.com/kubernetes/kubernetes/releases/).
 ...
 - Verwenden Sie ein Tool wie [7-Zip](http://www.7-zip.org/) Extrahieren des Archivs, und speichern Sie die Binärdateien in `C:\k\`.
...
Es ist eine Sammlung von Flannel Bereitstellungsskripts auf [dieses Microsoft-Repository](https://github.com/Microsoft/SDN/tree/master/Kubernetes/flannel/l2bridge) , mit dem Sie diesem Knoten zum Cluster hinzuzufügen.
...
Darüber hinaus sollten Sie sicherstellen, dass der Clustersubnetz (z. B. Kontrollkästchen "10.244.0.0/16") korrekt ist:
- [NET-conf.json](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/l2bridge/net-conf.json)


Wenn Sie [Ihre Windows-Knoten vorbereitet](#preparing-a-windows-node), und Ihr `c:\k` Directory folgendermaßen aussieht, können Sie die Knoten hinzuzufügen.

![Text](./media/flannel-directory.png)

#### <a name="join-node"></a>Verknüpfen von Knoten #### 
Um den Prozess zum Hinzufügen eines Windows-Knotens zu vereinfachen, müssen Sie nur ein Windows-Skript zum Starten führen `kubelet`, `kube-proxy`, `flanneld`, und Verbinden des Knotens.

> [!Note]
> [Dieses Skript](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/l2bridge/start.ps1) wird zusätzliche Dateien, die aktualisiert, z. B. herunterladen `flanneld` ausführbare Dateien und die [dockerfile-Datei für Infrastruktur-Pods](https://github.com/Microsoft/SDN/blob/master/Kubernetes/windows/Dockerfile) *, und führen Sie die für Sie*. Möglicherweise gibt es mehrere Powershell-Fenster wird geöffnet/sowie einige Sekunden Netzwerkausfall geschlossen, während der neuen externe vSwitch für das l2bridge-Pod-Netzwerk das erste Mal erstellt wird.
...
> [!tip]
> Sie notiert haben bereits Sie die Clustersubnetz, Dienst-Subnetz, und Kube-DNS-IP-Master-Linux [früheren](./creating-a-linux-master.md#collect-cluster-information)

Nach dem Ausführen dieser sollten Sie in der Lage sein:
  * Anzeigen von gehören Windows-Knoten mit `kubectl get nodes`
  * Finden Sie unter 3 Powershell-Fenstern öffnen, eine für `kubelet`, eine für `flanneld`, und eine für `kube-proxy`
  * Finden Sie unter Host-Agent-Prozesse für `flanneld`, `kubelet`, und `kube-proxy` auf dem Knoten


## <a name="joining-a-tor-cluster"></a>Beitreten zu einem Cluster ToR ##
> [!NOTE]
> Wenn Sie Flannel wie Ihr Netzwerk Lösung [zuvor](./network-topologies.md#flannel-in-host-gateway-mode)ausgewählt haben, können Sie diesen Abschnitt überspringen.

Zu diesem Zweck müssen Sie die Anweisungen für die [Einrichtung von Windows Server-Container auf Kubernetes für Upstream L3-Routingtopologie](https://kubernetes.io/docs/getting-started-guides/windows/#for-1-upstream-l3-routing-topology-and-2-host-gateway-topology)folgen. Dazu gehören, und stellen Sie sicher, dass Sie den upstream Router neu konfigurieren, z. B., die der Pod-CIDR Präfix eines Knotens zugewiesen, die jeweiligen Knoten IP zugeordnet ist.

Bei den neuen Knoten als "Bereit" aufgeführt ist, indem Sie `kubectl get nodes`, Kubelet + Kube-Proxy ausgeführt wird, und Sie den upstream ToR Router neu konfiguriert haben, sind Sie bereit für den nächsten Schritten.
...
Sie können auch wenn Ihnen keine passen Ihre Linux überspringen mit Schritt 6:
